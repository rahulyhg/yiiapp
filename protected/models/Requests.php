...
class Requests extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('requestType, status', 'numerical', 'integerOnly'=>true),
			array('senderId, receiverId', 'length', 'max'=>20),
			array('sendDate', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('requestId, senderId, receiverId, requestType, status, sendDate', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('requestType',$this->requestType);
